...
class SleepScreen extends StatelessWidget {
  final String uid;
  const SleepScreen({super.key, required this.uid});
  ...
  String _categoryOf(Map<String, dynamic> h) {
    ...
    if (emoji.contains('😴') || hasAny(['sleep', 'bed', 'rest'])) return "Sleep";
    return "Other";
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text("Sleep", style: AppText.h2),
        actions: [
          IconButton(
      ...
      body: StreamBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          final all = snap.data ?? [];
          final sleep = all.where((h) => _categoryOf(h) == "Sleep").toList();

          return ListView(
            padding: const EdgeInsets.fromLTRB(16, 14, 16, 18),
            children: [
              _SleepPanel(onOpenHabits: () {}),
              const SizedBox(height: 14),

              Row(
                children: [
                  Text("Sleep habits", style: AppText.h2.copyWith(fontSize: 18)),
                  const Spacer(),
                  TextButton(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => CategoryHabitsScreen(uid: uid, title: "Sleep", habits: sleep),
                        ),
                      );
                    },
              ...
              if (sleep.isEmpty)
                Container(
                  ...
                  child: Text("No sleep habits yet. Tap + to add one.", style: AppText.muted),
                )
              else
                ...sleep.take(20).map((h) => _SimpleHabitTile(habit: h)).toList(),
            ],
          );
        },
      ),
...
/// ✅ IMPORTANT:
/// Paste your existing _SleepPanel widget here OR import the file where it already exists.
class _SleepPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: const Text("Sleep Panel (use your existing widget)"),
    );
  }
}
